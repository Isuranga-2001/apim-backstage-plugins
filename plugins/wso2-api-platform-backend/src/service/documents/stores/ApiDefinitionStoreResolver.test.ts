...
import { ApiDefinitionStoreResolver } from './ApiDefinitionStoreResolver';
...
describe('ApiDefinitionStoreResolver', () => {
  const databaseStore = { name: 'database' } as unknown as ApiDefinitionStore;
  ...
  function resolverWith(entity: unknown) {
    ...
    return new ApiDefinitionStoreResolver(
  ...
  it('throws NotFoundError for an on-prem APIM entity (definitions are served from the catalog, not this store)', async () => {
    const resolver = resolverWith(
