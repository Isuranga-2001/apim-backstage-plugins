...
describe('DeleteDefinitionDialog', () => {
  const mockWso2Api = { deleteDefinition: jest.fn() };
  ...
  it('shows a permanence warning', () => {
    ...
    expect(screen.getByText(/permanently deleted/)).toBeDefined();
  });

  it('calls deleteDefinition and onDeleted on confirm', async () => {
    mockWso2Api.deleteDefinition.mockResolvedValue(undefined);
    ...
    expect(mockWso2Api.deleteDefinition).toHaveBeenCalledWith({
      kind: 'API',
  ...
  it('keeps the dialog open and shows an inline error on failure', async () => {
    mockWso2Api.deleteDefinition.mockRejectedValue(
